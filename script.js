const products = [
  {id:'silent-50', name:'Silent Disco — 50 Headphones', category:'Experiences', price:500, priceType:'fixed', desc:'Small-group silent disco package.', tag:'Popular', tone:'lime'},
  {id:'silent-100', name:'Silent Disco — 100 Headphones', category:'Experiences', price:900, priceType:'fixed', desc:'Balanced package for larger parties.', tag:'Fixed', tone:'lime'},
  {id:'silent-150', name:'Silent Disco — 150 Headphones', category:'Experiences', price:1200, priceType:'fixed', desc:'Mid-size silent event package.', tag:'Fixed', tone:'lime'},
  {id:'silent-200', name:'Silent Disco — 200 Headphones', category:'Experiences', price:1800, priceType:'fixed', desc:'Large crowd headphone package.', tag:'Fixed', tone:'lime'},
  {id:'silent-300', name:'Silent Disco — 300 Headphones', category:'Experiences', price:2500, priceType:'fixed', desc:'Maximum listed silent disco package.', tag:'Fixed', tone:'lime'},
  {id:'movie-12', name:'Outdoor Movie — 12 ft', category:'Outdoor Movies', price:549, priceType:'fixed', desc:'Screen, projector, two powered speakers and core playback gear.', tag:'Public price', tone:'orange'},
  {id:'movie-16', name:'Outdoor Movie — 16 ft', category:'Outdoor Movies', price:749, priceType:'fixed', desc:'Larger inflatable screen with the same core package.', tag:'Public price', tone:'orange'},
  {id:'movie-26', name:'Outdoor Movie — 26 ft', category:'Outdoor Movies', price:899, priceType:'fixed', desc:'Community-scale outdoor cinema package.', tag:'Public price', tone:'orange'},
  {id:'movie-40', name:'Outdoor Movie — 40 ft', category:'Outdoor Movies', price:1899, priceType:'fixed', desc:'Large-format outdoor movie experience.', tag:'Public price', tone:'orange'},
  {id:'ipad-mirror', name:'iPad Mirror Photo Booth', category:'Photo Booths', price:799, priceType:'starting', desc:'Interactive iPad mirror setup. Listed as starting at.', tag:'From', tone:'blue'},
  {id:'magic-air', name:'Magic Mirror Air', category:'Photo Booths', price:1399, priceType:'starting', desc:'Full magic mirror photo booth experience.', tag:'From', tone:'blue'},
  {id:'magic-frame', name:'Magic Mirror + Frame', category:'Photo Booths', price:1699, priceType:'starting', desc:'Framed magic mirror experience.', tag:'From', tone:'blue'},
  {id:'photo-360', name:'360 Photo Booth', category:'Photo Booths', price:1499, priceType:'starting', desc:'360-degree video/photo platform.', tag:'From', tone:'blue'},
  {id:'small-audio', name:'Small Audio Package', category:'Audio', price:null, priceType:'quote', desc:'2 powered speakers, wireless/wired mic, small mixer.', tag:'Quote', tone:''},
  {id:'medium-audio', name:'Medium Audio Package', category:'Audio', price:null, priceType:'quote', desc:'4 speakers, wireless mics and 16-channel mixer.', tag:'Quote', tone:''},
  {id:'large-audio', name:'Large Audio Package', category:'Audio', price:null, priceType:'quote', desc:'Line arrays, subs, multi-channel mixer and support.', tag:'Quote', tone:''},
  {id:'tv-65', name:'Samsung 65” 4K TV', category:'Video & Displays', price:null, priceType:'quote', desc:'4K UHD display for presentations and events.', tag:'Quote', tone:''},
  {id:'tv-75', name:'Samsung 75” 4K TV', category:'Video & Displays', price:null, priceType:'quote', desc:'Larger 4K UHD display for event viewing.', tag:'Quote', tone:''},
  {id:'led-12', name:'Indoor LED Wall — 12 × 7 ft', category:'LED Walls', price:null, priceType:'quote', desc:'2.67 mm LED wall with processor and audio package.', tag:'Quote', tone:''},
  {id:'led-16', name:'Indoor LED Wall — 16 × 9 ft', category:'LED Walls', price:null, priceType:'quote', desc:'Large indoor LED wall with controller package.', tag:'Quote', tone:''},
  {id:'stream-basic', name:'Live Streaming — Basic', category:'Live Streaming', price:null, priceType:'quote', desc:'1 × 4K camera, streaming laptop, switcher and mixer.', tag:'Quote', tone:''},
  {id:'stream-pro', name:'Live Streaming — Pro', category:'Live Streaming', price:null, priceType:'quote', desc:'3 × 4K cameras and multi-angle production setup.', tag:'Quote', tone:''},
  {id:'wired-mic', name:'Wired Microphone', category:'Microphones', price:null, priceType:'quote', desc:'Reliable cable-connected microphone for event use.', tag:'Quote', tone:''},
  {id:'lapel-mic', name:'Lapel Microphone', category:'Microphones', price:null, priceType:'quote', desc:'Hands-free clip-on mic for presentations and ceremonies.', tag:'Quote', tone:''},
  {id:'iem', name:'In-Ear Monitors', category:'Microphones', price:null, priceType:'quote', desc:'Performer monitoring solution.', tag:'Quote', tone:''}
];

const bundles = [
  {id:'wedding', title:'Wedding + Reception', tone:'bundle-wedding', label:'WEDDING', desc:'Ceremony clarity and reception energy, built around your guest count.', items:['Main speaker package','2–4 wireless microphones','Mixer + signal kit','Display for slideshow','Optional lighting'], cta:'Customize wedding'},
  {id:'corporate', title:'Corporate Event', tone:'bundle-corporate', label:'CORPORATE', desc:'Presentation-ready audio, video and streaming support for meetings and launches.', items:['Presentation sound','Wireless microphones','Projector / TV','Signal switching','Optional live stream'], cta:'Build corporate setup'},
  {id:'school', title:'School Event', tone:'bundle-school', label:'SCHOOL', desc:'Flexible AV for ceremonies, recognition programs, dances and outdoor activities.', items:['Speaker package','Wireless mics','Mixer + stands','Optional lighting','Movie / silent disco add-on'], cta:'Plan school event'},
  {id:'concert', title:'Concert + Live Performance', tone:'bundle-concert', label:'CONCERT', desc:'Production-focused setup for stage audio, performers and visual systems.', items:['Line array PA','Subwoofers','Stage monitors','Digital mixer','Tech support'], cta:'Build concert setup'},
  {id:'party', title:'Private Party', tone:'bundle-party', label:'PARTY', desc:'A compact entertainment setup for birthdays, celebrations and social events.', items:['Powered speakers','Wireless mic','Mixer','Optional DJ gear','Optional photo booth'], cta:'Customize party'},
  {id:'movie', title:'Outdoor Movie Night', tone:'bundle-movie', label:'MOVIE NIGHT', desc:'Pick the screen size and turn a backyard, school or community field into a cinema.', items:['Inflatable movie screen','Sony laser projector','2 powered speakers','Audio mixer','Playback laptop / DVD'], cta:'Choose screen size'}
];
...
function getHash(){ return location.hash.replace('#','').split('?')[0] || 'home'; }
function productPriceText(p){ if(p.priceType==='quote') return 'Quote required'; if(p.priceType==='starting') return `From ${money(p.price)}`; return money(p.price); }
function filteredProducts(){
  const q = state.search.trim().toLowerCase();
  return products.filter(p => (state.category==='All'||p.category===state.category) && (!q||`${p.name} ${p.category} ${p.desc}`.toLowerCase().includes(q)));
}
...
function home(){
 return `<div class="page">
  ...
  <section class="section"><div class="section-head"><div><span class="eyebrow">POPULAR NOW</span><h2>Public pricing,<br>where it exists.</h2></div><a class="btn btn-light" href="#rentals">See all rentals</a></div><div class="product-grid">${products.filter(p=>['movie-12','movie-16','silent-100','photo-360'].includes(p.id)).map(productCard).join('')}</div></section>
  ...
  <section class="testimonial"><div><span class="eyebrow">DESIGN PRINCIPLE</span><div class="quote-author">A client-facing experience should feel like planning the event, not filling out a spreadsheet.</div></div><blockquote>“The website should sell the <em>outcome</em> first — then explain the equipment.”</blockquote></section>
 </div>`;
}

function rentals(){
 const cats=['All','Audio','Microphones','Video & Displays','LED Walls','Experiences','Outdoor Movies','Photo Booths','Live Streaming'];
...
function builder(){
 const items=Object.entries(state.selected).map(([id,qty])=>{const p=products.find(x=>x.id===id); return p?{p,qty}:null}).filter(Boolean);
 const known=items.reduce((s,{p,qty})=>s+(p.price||0)*qty,0);
 const quoteOnly=items.some(({p})=>p.priceType==='quote'||p.priceType==='starting');
 const suggested=state.eventType==='Wedding'?['medium-audio','wired-mic','lapel-mic','tv-65']:state.eventType==='Concert'?['large-audio','iem','wired-mic','led-16']:state.eventType==='School Event'?['medium-audio','wired-mic','movie-16','silent-100']:['small-audio','wired-mic','tv-65','stream-basic'];
 const builderProducts=products.filter(p=>suggested.includes(p.id)||['movie-12','movie-16','silent-100','photo-360'].includes(p.id));
...
function addProduct(id,delta=1){ state.selected[id]=(state.selected[id]||0)+delta; if(state.selected[id]<=0) delete state.selected[id]; render(); showToast(delta>0?'Added to your setup':'Removed from your setup'); }

function bind(){
 document.querySelectorAll('[data-cat]').forEach(b=>b.addEventListener('click',()=>{state.category=b.dataset.cat;state.search='';document.querySelector('#rentalSearch')?.focus();render();}));
 document.querySelector('#rentalSearch')?.addEventListener('input',e=>{state.search=e.target.value;const scroll=window.scrollY;app.innerHTML=rentals();bind();window.scrollTo(0,scroll)});
 document.querySelectorAll('[data-add]').forEach(b=>b.addEventListener('click',()=>addProduct(b.dataset.add)));
 document.querySelectorAll('[data-plus]').forEach(b=>b.addEventListener('click',()=>addProduct(b.dataset.plus,1)));
 document.querySelectorAll('[data-minus]').forEach(b=>b.addEventListener('click',()=>addProduct(b.dataset.minus,-1)));
 document.querySelectorAll('[data-remove]').forEach(b=>b.addEventListener('click',()=>{delete state.selected[b.dataset.remove];render();showToast('Removed from your setup')}));
 document.querySelectorAll('[data-event]').forEach(b=>b.addEventListener('click',()=>{state.eventType=b.dataset.event;render();}));
 document.querySelectorAll('[data-bundle]').forEach(b=>b.addEventListener('click',()=>{state.bundle=b.dataset.bundle;const match=bundles.find(x=>x.id===b.dataset.bundle); if(match){ if(match.id==='movie') state.eventType='Outdoor Movie'; else if(match.id==='wedding') state.eventType='Wedding'; else if(match.id==='concert') state.eventType='Concert'; else if(match.id==='school') state.eventType='School Event'; else if(match.id==='corporate') state.eventType='Corporate Event'; else state.eventType='Private Party'; } state.builderStep=1; location.hash='builder';}));
 document.querySelector('[data-builder-next]')?.addEventListener('click',()=>{if(state.builderStep===2){state.guests=parseInt(document.querySelector('#guestInput')?.value||120);state.venue=document.querySelector('#venueInput')?.value||'Indoor';state.date=document.querySelector('#dateInput')?.value||'';}state.builderStep=Math.min(4,state.builderStep+1);render();});
 document.querySelector('[data-builder-prev]')?.addEventListener('click',()=>{state.builderStep=Math.max(1,state.builderStep-1);render();});
 document.querySelector('#quoteForm')?.addEventListener('submit',e=>{e.preventDefault();document.querySelector('#quoteCard')?.classList.add('submitted');showToast('Quote request captured in the mockup');});
}

const mobileMenu=document.getElementById('mobileMenu');
document.getElementById('menuBtn')?.addEventListener('click',()=>mobileMenu.style.display=mobileMenu.style.display==='flex'?'none':'flex');
...
document.getElementById('searchBtn')?.addEventListener('click',()=>{searchOverlay.hidden=false;searchInput.focus();renderSearch('');});
document.getElementById('closeSearch')?.addEventListener('click',()=>searchOverlay.hidden=true);
searchOverlay?.addEventListener('click',e=>{if(e.target===searchOverlay)searchOverlay.hidden=true});
searchInput?.addEventListener('input',e=>renderSearch(e.target.value));
function renderSearch(q){
 const r=products.filter(p=>!q||`${p.name} ${p.category} ${p.desc}`.toLowerCase().includes(q.toLowerCase())).slice(0,10);
 searchResults.innerHTML=r.length?r.map(p=>`<button class="search-item" data-search-item="${p.id}"><span><strong>${p.name}</strong><br><span>${p.category}</span></span><span>${productPriceText(p)} →</span></button>`).join(''):`<div class="empty">No matching rentals.</div>`;
 searchResults.querySelectorAll('[data-search-item]').forEach(b=>b.addEventListener('click',()=>{state.search=b.querySelector('strong').textContent;state.category='All';searchOverlay.hidden=true;location.hash='rentals';}));
}

window.addEventListener('hashchange',()=>{ if(getHash()!=='builder') state.builderStep=1; render(); });
