...
function builder() {
  ...
  const recommendedIds = recommendations[state.eventType] || recommendations.Wedding;
  const recommendedProducts = recommendedIds
    .map((id) => products.find((p) => p.id === id))
    .filter(Boolean);
  const otherProducts = products.filter((p) => !recommendedIds.includes(p.id));
  const miniProduct = (p, recommended = false) => `<div class="mini-product ${recommended ? "is-recommended" : ""}"><div class="mini-product-heading"><h4>${p.name}</h4>${recommended ? '<span class="recommendation-tag">Recommended</span>' : ""}</div><p>${productPriceText(p)}</p><div class="mini-product-row"><span style="font-size:11px;color:var(--muted)">${p.category}</span><div class="qty-controls"><button class="qty-btn" data-minus="${p.id}">−</button><span class="qty">${state.selected[p.id] || 0}</span><button class="qty-btn" data-plus="${p.id}">+</button></div></div></div>`;
